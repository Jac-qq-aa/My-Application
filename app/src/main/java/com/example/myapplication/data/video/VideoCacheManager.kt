 ...
 *
 * 设计目标：
 * 1. 优先保证视频能立即播放：缓存命中时播放本地文件，否则先在线播放；
 * 2. 后续再次播放同一个 URL 时，直接返回本地文件 Uri，避免重复走网络；
 * 3. 在线播放时在后台把 MP4 下载到 App 私有缓存目录，缓存失败不阻断播放。
 *
...
object VideoCacheManager {
    ...
    /**
     * 获取可播放的视频 Uri。缓存命中时返回本地文件；未命中时立即返回远程 URL。
     *
     * suspend 表示这个函数会做磁盘和网络 IO，必须放在协程里调用。
     * withContext(Dispatchers.IO) 会把磁盘检查切到 IO 线程池，避免阻塞 Compose 主线程。
     */
    suspend fun getPlayableVideoUri(
        context: Context,
        remoteUrl: String
    ): Uri = withContext(Dispatchers.IO) {
        getCachedVideoUri(context, remoteUrl) ?: Uri.parse(remoteUrl)
    }

    suspend fun getCachedVideoUri(
        context: Context,
        remoteUrl: String
    ): Uri? = withContext(Dispatchers.IO) {
