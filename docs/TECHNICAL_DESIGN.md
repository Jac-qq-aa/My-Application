...
- 显式开启内存缓存、磁盘缓存和网络缓存。
- 图片加载中展示渐变占位。
- 图片加载失败时自动展示 `drawable-nodpi` 中的本地广告封面。
- 本地内置 5 张广告封面，保证手机无法访问外网图片时仍能稳定演示。
- 渲染策略改为本地封面先显示，网络图片加载成功后覆盖；划出屏幕再划回来时至少能立即看到本地封面。
- 网络图片如果成功加载，会继续由 Coil 写入内存缓存和磁盘缓存。
...
| 方案 | 优点 | 缺点 | 结论 |
| --- | --- | --- | --- |
| 本地 Mock AI 字段 | 稳定、无网络依赖 | 不是真实 AI | 已作为初始展示 |
| 调本地 Qwen/Ollama | 真实大模型能力，可本地免费部署 | 需要本机服务、真机需配置局域网地址 | 当前采用 |
...
- Qwen 首次失败后，本次 App 运行直接降级为本地规则，避免每条广告重复等待超时。
- Qwen 输出解析会截取首个 JSON 对象，能容忍模型在 JSON 前后误加少量解释文本。
