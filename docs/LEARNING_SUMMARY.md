...
通过本次训练营项目，我对 Jetpack Compose 信息流开发、客户端状态管理、AI 能力接入以及埋点统计有了更完整的实践认识。这个项目一开始看起来只是实现一个广告列表，但真正开发过程中发现，信息流场景的复杂度不在于"把卡片画出来"，而在于卡片类型扩展、列表性能、页面状态同步、AI 输出稳定性和曝光口径这些细节问题。
